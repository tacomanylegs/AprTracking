...
async function performScrape() {
  ...
  try {
    ...
    if (mmtApr) results.push({ name: CONFIG.mmt.name, apr: mmtApr, url: CONFIG.mmt.url });

    // 2. TakaraLend USDT
    const usdtApr = await scrapeInTab(targetWindowId, CONFIG.usdt.url, 'takaralend');
    if (usdtApr) results.push({ name: CONFIG.usdt.name, apr: usdtApr, url: CONFIG.usdt.url });

    // 3. TakaraLend USDC
    const usdcApr = await scrapeInTab(targetWindowId, CONFIG.usdc.url, 'takaralend');
    if (usdcApr) results.push({ name: CONFIG.usdc.name, apr: usdcApr, url: CONFIG.usdc.url });

    // 4. Volos
    const volosData = await scrapeInTab(targetWindowId, CONFIG.volos.url, 'volos');
    if (volosData) {
      if (volosData.vault_1) results.push({ name: 'Volos Vault #1', apr: volosData.vault_1, url: CONFIG.volos.url });
      if (volosData.vault_2) results.push({ name: 'Volos Vault #2', apr: volosData.vault_2, url: CONFIG.volos.url });
    }
    ...
    await new Promise(resolve => {
      chrome.tabs.onUpdated.addListener(function listener(tid, info) {
        if (tid === tabId && info.status === 'complete') {
          ...
          setTimeout(resolve, 4000); // 給予 SPA 渲染時間
        }
      });
    });
...
function getAprFromPage(type) {
  ...
  if (type === 'volos') {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/Stable\s+Vault\s+#2\b/) && !line.includes('#12') && !line.includes('#20')) {
        for (let j = 1; j <= 10 && i + j < lines.length; j++) {
          const percentMatch = lines[i + j].match(/(\d+\.\d+)%/);
          if (percentMatch) {
